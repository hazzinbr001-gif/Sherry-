 ...
 *
 * Location strategy:
 *   Villages are free-text but fenced at input (Title-Case, hyphens, no spaces).
 *   Any location with ≤3 survey records is treated as unconfirmed and excluded
 *   from by_location processing — it won't appear in reports.
 */
...
function computeSurveyMetrics(records) {
  ...
  // Single-pass group: build grouped data once, share across all sub-functions
  const grouped = _groupRecords(records);

  const metrics = {
    summary:         _computeSummary(records, grouped),
    by_location:     _computeByLocation(grouped),
    by_interviewer:  _computeByInterviewer(grouped),
    ...
    recommendations: [],
    unconfirmed_locations: grouped.unconfirmed,  // visible in admin for review
  };
...
// ─────────────────────────────────────────────────────────────
//  SINGLE-PASS GROUPING  (fixes Performance Scaling)
//  Iterates records ONCE → produces:
//    grouped.byLocation   — confirmed locations only (count > 3)
//    grouped.byInterviewer
//    grouped.unconfirmed  — locations with ≤3 records (excluded from reports)
// ─────────────────────────────────────────────────────────────
const MIN_LOCATION_COUNT = 3; // locations must appear more than this to be confirmed

function _groupRecords(records) {
  const locBuckets = {};   // normKey → { canonical, records[] }
  const ivBuckets  = {};   // interviewer → { records[] }

  records.forEach(r => {
    // ── location ──
    const rawLoc = _resolveLocation(r);
    const key    = _normLocKey(rawLoc);
    if (!locBuckets[key]) locBuckets[key] = { canonical: rawLoc, records: [] };
    // Keep the most-seen capitalised form as canonical (majority wins)
    // We track counts per raw form for that
    locBuckets[key].records.push(r);

    // ── interviewer ──
    const iv = (_field(r, 'interviewer_name') || _field(r, 'interviewer') || 'Unknown').trim();
    if (!ivBuckets[iv]) ivBuckets[iv] = { records: [] };
    ivBuckets[iv].records.push(r);
  });

  // Resolve canonical name per location (most-seen capitalised form)
  Object.keys(locBuckets).forEach(key => {
    const formCounts = {};
    locBuckets[key].records.forEach(r => {
      const raw = _resolveLocation(r) || 'Unknown';
      formCounts[raw] = (formCounts[raw] || 0) + 1;
    });
    locBuckets[key].canonical = Object.keys(formCounts)
      .sort((a, b) => formCounts[b] - formCounts[a])[0];
  });

  // Split confirmed vs unconfirmed by count threshold
  const byLocation   = {};
  const unconfirmed  = {};

  Object.keys(locBuckets).forEach(key => {
    const bucket = locBuckets[key];
    const name   = bucket.canonical || key;
    if (bucket.records.length > MIN_LOCATION_COUNT) {
      byLocation[name] = bucket.records;
    } else {
      unconfirmed[name] = bucket.records.length;
    }
  });

  return { byLocation, byInterviewer: ivBuckets, unconfirmed };
}

// ─────────────────────────────────────────────────────────────
//  LOCATION HELPERS  (fixes Location Structure)
//  Single source of truth for resolving a record's location.
//  _normLocKey normalises for grouping only — display uses canonical.
// ─────────────────────────────────────────────────────────────

/**
 * Resolve location from a record.
 * Checks interview_location first, falls back to location field.
 */
function _resolveLocation(record) {
  return (
    (_field(record, 'interview_location') || _field(record, 'location') || '').trim()
  ) || null;
}

/**
 * Normalise a village string to a stable grouping key.
 * Lowercase + collapse whitespace + remove stray punctuation.
 * e.g. "Nyamache-Village" → "nyamache-village"
 *      "nyamache village" → "nyamache village"  (spaces allowed in key only)
 */
...
// ─────────────────────────────────────────────────────────────
//  ANSWER HELPERS  (fixes Data Consistency)
//
//  _isYes  — explicit affirmative
//  _isNo   — explicit negative  (NEW)
//  _isAnswered — field has a real value (not null/undefined/""/N/A)  (NEW)
//
//  Rule: use !_isAnswered() to detect missing data.
//        use _isNo() to detect confirmed negatives.
//        Never use !_isYes() as a proxy for "No" — it swallows nulls.
// ─────────────────────────────────────────────────────────────
function _isYes(val) {
...
function _isNo(val) {
  return ['no', 'No', 'NO', 'n', 'N', false, '0', 0].includes(val);
}

function _isAnswered(val) {
  if (val === null || val === undefined) return false;
  const s = String(val).trim().toLowerCase();
  return s !== '' && s !== 'n/a' && s !== 'na' && s !== 'none' && s !== 'unknown';
}
...
//  SUMMARY
// ─────────────────────────────────────────────────────────────
function _computeSummary(records, grouped) {
  ...
  return {
    ...
    total_locations:        Object.keys(grouped.byLocation).length,
    unconfirmed_locations:  Object.keys(grouped.unconfirmed).length,
...
// ─────────────────────────────────────────────────────────────
//  BY LOCATION  (confirmed only, derives from shared grouped data)
//  Metrics use the same field logic as global metrics — no separate
//  re-implementation — so numbers always match.
// ─────────────────────────────────────────────────────────────
function _computeByLocation(grouped) {
  const result = {};

  Object.keys(grouped.byLocation).forEach(loc => {
    const recs = grouped.byLocation[loc];
    const n    = recs.length;

    result[loc] = {
      surveys:             n,
      population:          recs.reduce((s, r) =>
        s + (parseInt(_field(r, 'a_tot_m')) || 0) + (parseInt(_field(r, 'a_tot_f')) || 0), 0),

      // Infrastructure — consistent with _computeInfrastructure()
      pct_permanent_house: _pct(recs.filter(r => _field(r, 'b_type') === 'Permanent').length, n),
      pct_electricity:     _pct(recs.filter(r =>
        (_field(r, 'b_light') || '').toLowerCase().includes('electric')).length, n),

      // Sanitation — consistent with _computeInfrastructure()
      pct_pit_latrine:     _pct(recs.filter(r =>
        _isYes(_field(r, 'g_latrine') || _field(r, 'latrine'))).length, n),
      pct_water_treated:   _pct(recs.filter(r => _isYes(_field(r, 'h_treat'))).length, n),

      // Health — consistent with _computeHealthMetrics()
      pct_hiv_aware:       _pct(recs.filter(r => _isYes(_field(r, 'f_heard'))).length, n),

      // Nutrition — consistent with _computeNutrition()
      pct_food_secure:     _pct(recs.filter(r => _isYes(_field(r, 'e_enough'))).length, n),

      // Child health — consistent with _computeMaternalChild()
      pct_immunised:       _pct(recs.filter(r => _isYes(_field(r, 'd_immun'))).length, n),

      // Environmental — consistent with _computeEnvironmental()
...
//  BY INTERVIEWER
// ─────────────────────────────────────────────────────────────
function _computeByInterviewer(grouped) {
  const result = {};

  Object.keys(grouped.byInterviewer).forEach(iv => {
    const recs = grouped.byInterviewer[iv].records;
    const n    = recs.length;
    const complete = recs.filter(r =>
      _resolveLocation(r) && _field(r, 'interview_date') && _field(r, 'a_age')
    ).length;
...
function _computeHealthMetrics(records) {
  ...
  records.forEach(r => {
    ...
    String(ill).split(',').forEach(x => {
      const k = x.trim();
      if (k && _isAnswered(k)) illCounts[k] = (illCounts[k] || 0) + 1;
    });
  });
  ...
  // Count only answered records for denominator where relevant
  const answeredIll  = records.filter(r => _isAnswered(_field(r, 'c_ill') || _field(r, 'illnesses')));
  const answeredHiv  = records.filter(r => _isAnswered(_field(r, 'f_heard')));
  const answeredTest = records.filter(r => _isAnswered(_field(r, 'f_tested')));

  return {
    pct_with_illness:     _pct(answeredIll.filter(r => {
      const ill = String(_field(r, 'c_ill') || _field(r, 'illnesses') || '').trim();
      return ill.length > 0 && ill.toLowerCase() !== 'none';
    }).length, answeredIll.length || n),
    ...
    pct_hiv_aware:        _pct(answeredHiv.filter(r => _isYes(_field(r, 'f_heard'))).length, answeredHiv.length || n),
    pct_hiv_not_aware:    _pct(answeredHiv.filter(r => _isNo(_field(r, 'f_heard'))).length, answeredHiv.length || n),
    pct_hiv_tested:       _pct(answeredTest.filter(r => _isYes(_field(r, 'f_tested'))).length, answeredTest.length || n),
...
function _computeInfrastructure(records) {
  ...
  return {
    ...
    pct_iron_roof:       _pct(records.filter(r =>
      (_field(r, 'b_roof') || '').toLowerCase().includes('iron')).length, n),
    pct_cemented_floor:  _pct(records.filter(r =>
      (_field(r, 'b_floor') || '').toLowerCase().includes('cement')).length, n),
    pct_electricity:     _pct(records.filter(r =>
      (_field(r, 'b_light') || '').toLowerCase().includes('electric')).length, n),
    ...
    pct_pit_latrine:     _pct(records.filter(r =>
      _isYes(_field(r, 'g_latrine') || _field(r, 'latrine'))).length, n),
...
function _computeNutrition(records) {
  ...
  // Use _isAnswered to get correct denominator for e_enough and e_skip
  const answeredEnough = records.filter(r => _isAnswered(_field(r, 'e_enough')));
  const answeredSkip   = records.filter(r => _isAnswered(_field(r, 'e_skip')));

  return {
    pct_food_sufficient: _pct(
      answeredEnough.filter(r => _isYes(_field(r, 'e_enough'))).length,
      answeredEnough.length || n
    ),
    pct_skipping_meals:  _pct(
      answeredSkip.filter(r => _isYes(_field(r, 'e_skip'))).length,
      answeredSkip.length || n
    ),
...
function _computeRiskProfiles(records) {
  return records.map(r => {
    ...
    // Sanitation (30 pts)
    if (_isNo(_field(r, 'g_latrine') || _field(r, 'latrine'))) { factors.push('No pit latrine'); score += 15; }
    if (_isNo(_field(r, 'h_treat')))                            { factors.push('Water not treated'); score += 15; }

    // Health (25 pts)
    if (_isNo(_field(r, 'f_heard')))    { factors.push('HIV unaware'); score += 10; }
    ...
    if (_isNo(_field(r, 'e_enough')))   { factors.push('Food insufficient'); score += 10; }

    // Maternal & Child (15 pts)
    if (_isNo(_field(r, 'd_immun')) && parseInt(_field(r, 'd_u5') || 0) > 0)
    ...
    if (_isNo(_field(r, 'h_treat')) &&
        (_field(r, 'h_wsrc') || '').toLowerCase().includes('unprotect'))
    ...
    return {
      survey_id:   r.id,
      location:    _resolveLocation(r) || 'Unknown',
...
function _computeDataQuality(records) {
  ...
  // Use _isAnswered to detect truly missing data (not "No" answers)
  const missingCritical  = records.filter(r => criticalFields.some(f => !_isAnswered(_field(r, f)))).length;
  const missingImportant = records.filter(r => importantFields.some(f => !_isAnswered(_field(r, f)))).length;

  const completeness = criticalFields.map(f => ({
    field:  f,
    filled: records.filter(r => _isAnswered(_field(r, f))).length,
    pct:    _pct(records.filter(r => _isAnswered(_field(r, f))).length, n),
  }));
...
function _generateRecommendations(metrics) {
  ...
  if (metrics.health.pct_hiv_aware < 90) {
    recs.push({
      ...
      issue: `${metrics.health.pct_hiv_not_aware}% of households confirmed no HIV awareness`,
      action: 'Schedule community HIV awareness sessions and distribute education materials',
      households_affected: Math.round(metrics.summary.total_surveys * (metrics.health.pct_hiv_not_aware / 100)),
    });
  }
...
function _countField(records, field) {
  const c = {};
  records.forEach(r => {
    const v = _field(r, field);
    if (!_isAnswered(v)) return;  // skip nulls/blanks in breakdowns
...
function admRenderEnhancedMetrics() {
  ...
  const unconfirmedCount = Object.keys(r.unconfirmed_locations || {}).length;
  ...
  panel.innerHTML = `
    <div style="padding:16px;border-bottom:1px solid #eee;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-bottom:16px;">
        ...
        ${_metricCard('Confirmed Locations', r.summary.total_locations, '#f3e5f5')}
      </div>

      ${unconfirmedCount > 0 ? `
      <div style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;padding:10px 12px;margin-bottom:14px;font-size:.75rem;color:#795548;">
        ⚠️ <strong>${unconfirmedCount} unconfirmed location(s)</strong> excluded from reports (≤${MIN_LOCATION_COUNT} surveys each):
        ${Object.entries(r.unconfirmed_locations).map(([loc, cnt]) => `<span style="background:#fffde7;border-radius:4px;padding:1px 6px;margin:2px;">${loc} (${cnt})</span>`).join(' ')}
      </div>` : ''}
